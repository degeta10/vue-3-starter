<template>
    <div class="min-h-screen flex items-center justify-center bg-gray-100">
        <div class="bg-white p-8 rounded shadow-md w-full max-w-md">
            ...
            <div v-if="profileApiErrors?.errors?.length === 0" class="mb-4 p-3 bg-red-100 text-red-700 rounded">
                {{ profileApiErrors.message }}
            </div>
            <div v-if="profileApiSuccess?.message === $t('profile.updateSuccess')"
            ...
            <form @submit.prevent="updateProfile">
                <div class="mb-4">
                    ...
                    <p v-if="profileApiErrors?.email" class="text-red-500 text-sm mt-1">{{ profileApiErrors.email }}</p>
                </div>
                <div class="mb-4">
                    ...
                    <p v-if="profileApiErrors?.name" class="text-red-500 text-sm mt-1">{{ profileApiErrors.name }}</p>
                </div>
                <div class="mb-6">
                    <label class="block text-gray-700 mb-2" for="password">{{ $t('profile.password') }}</label>
                ...
                <div class="mb-6">
                    <label class="block text-gray-700 mb-2" for="password_confirmation">{{
                        $t('profile.passwordConfirmation') }}</label>
                ...
                <button class="w-full bg-red-500 text-white py-2 rounded hover:bg-red-600 transition duration-200"
                    type="submit">
